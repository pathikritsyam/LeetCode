...
public:
    ...
    void dfs(vector<vector<char>>& board, int r, int c, TrieNode* cur,string res) {
        ...
        if (cur->endsHere!=false) {
            ...
            ans.push_back(res);
            res.pop_back();
            cur->endsHere = false; // Avoid duplication!
        }
        res.push_back(board[r][c]);
        board[r][c] = '#'; // mark as visited!
        for (int i = 0; i < 4; ++i) dfs(board, r + DIR[i], c + DIR[i+1], cur,res);
        board[r][c] = orgChar;
        res.pop_back();// restore org state
    }
};